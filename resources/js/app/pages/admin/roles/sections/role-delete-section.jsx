...
import { delete_roles_thunk } from '../redux/roles-thunk';
import store from '@/app/store/store';

export default function RoleDeleteSection({data}) {
    ...
    const handleMouseLeave = () => {
      ...
      function deleteRoles(id) {
        store.dispatch(delete_roles_thunk(id))
        closeModal();
    }
    return (
        <div>
            <button
                ...
                {tooltipVisible && (
                <span className="tooltip bg-black text-white text-md rounded-xl p-3 absolute z-50" style={{ top: tooltipPosition.y + window.pageYOffset, left: tooltipPosition.x }}>Delete Role</span>
            )}
            </button>
            <Modal
                open={open}
            ...
            >
                ...
                <div className="flex border-gray-200 rounded-b  mt-3.5">
                    <button data-modal-hide="default-modal" type="button" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center  " onClick={() => deleteRoles(data.id)}>Yes, Proceed</button>
